...
import h5py
import pprint
os.chdir(r"B:\Desktop\Moberg\my_code\active")
from hdf5_classes import HDF5Convert
```

```{python}
# Instantiate the HDF5Convert class
hdf5file = h5py.File(r"B:\Desktop\Moberg\data\hdf5\example.h5", "r")
conv_object = HDF5Convert(hdf5file=hdf5file)
```
...
**get_numpy_values(dataset_name=None, dataset_path=None)**

*DESCRIPTION:*

Returns a NumPy Array of the values in the dataset.           
...
```{python}
np_values = conv_object.get_numpy_values(dataset_name="Dataset A")
```
```{python, echo=FALSE}
pprint.pp(np_values)
```
...
**get_pandas_values(dataset_name=None, dataset_path=None)**

*DESCRIPTION:*

Returns a Pandas DataFrame of the values in the dataset.           

*PARAMETERS:*

- **dataset_name**: str

    name of the dataset to get the Pandas DataFrame from
    
    -or-
    
- **dataset_path**: str

    path of the dataset to get the Pandas DataFrame from

*RETURNS:*

- **pd_values**: Pandas DataFrame 

    Pandas DataFrame of dataset values
    
*EXAMPLE:*

```{python}
pd_values = conv_object.get_pandas_values(dataset_name="Dataset A")
```
```{python, echo=FALSE}
pprint.pp(pd_values)
```
...
**get_metadata(group_name, dataset_name)**
...
```{python}
hdf5_path = conv_object.get_path(group_name="Group 1")
```
```{python, echo=FALSE}
print(hdf5_path)
```
...
```{python}
group_metadata = conv_object.get_metadata(group_name="Group 1")
```
```{python, echo=FALSE}
...
```{python}
dataset_metadata = conv_object.get_metadata(dataset_name="Dataset A")
```
```{python, echo=FALSE}
